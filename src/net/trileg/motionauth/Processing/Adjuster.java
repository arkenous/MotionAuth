...
public class Adjuster {
  ...
  public ArrayList<float[][][]> adjust(float[][][] linearAcceleration, float[][][] gyroscope) {
    ...
    for (float[][] aLinearAcceleration : linearAcceleration)
      linearAccelerationLengthList.add(aLinearAcceleration[0].length);
    for (float[][] aGyroscope : gyroscope)
      gyroscopeLengthList.add(aGyroscope[0].length);
    ...
    float[][][] linearAccelerationArray
        = new float[linearAcceleration.length][linearAcceleration[0].length][maxLength];
    float[][][] gyroscopeArray
        = new float[gyroscope.length][gyroscope[0].length][maxLength];
    ...
    for (int time = 0; time < linearAcceleration.length; time++)
      if (time == maxTime)
        ...
        if (linearAccelerationLengthList.get(time) > gyroscopeLengthList.get(time)) {
          ...
          for (int axis = 0; axis < NUM_AXIS; axis++) {
            for (int length = 0; length < maxLength; length++) {
              gyroscopeArray[time][axis][length] = gyroscope[time][axis][length];
              if (length < maxLength - diff)
                linearAccelerationArray[time][axis][length]
                    = linearAcceleration[time][axis][length];
        ...
        for (int axis = 0; axis < NUM_AXIS; axis++) {
          for (int length = 0; length < maxLength; length++) {
            if (length < maxLength - linearAccelerationDiff)
              linearAccelerationArray[time][axis][length] = linearAcceleration[time][axis][length];
            else linearAccelerationArray[time][axis][length] = 0;

            if (length < maxLength - gyroscopeDiff)
              gyroscopeArray[time][axis][length] = gyroscope[time][axis][length];
  ...
  public float[][] adjust(float[][] input, int registeredDataLength) {
    ...
    if (registeredDataLength < input[0].length) {
      ...
      for (int axis = 0; axis < NUM_AXIS; axis++)
        System.arraycopy(input[axis], 0, result[axis], 0, registeredDataLength);
      ...
      for (int axis = 0; axis < NUM_AXIS; axis++)
        for (int length = 0; length < registeredDataLength; length++)
          if (length < registeredDataLength - diff) result[axis][length] = input[axis][length];
          else result[axis][length] = 0;
    } else
      for (int axis = 0; axis < NUM_AXIS; axis++)
        System.arraycopy(input[axis], 0, result[axis], 0, registeredDataLength);
